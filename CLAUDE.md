...
- Robustness: a hardware watchdog (`WDT.h`, `WDT_TIMEOUT_MS` ≈5.5s, the RA4M1 max) is refreshed in `loop()` and the portal loop; every blocking network call is bounded below it (`WIFI_BEGIN_WAIT`, `MQTT_TCP_WAIT` + `MQTT_ACK_WAIT`). After a watchdog reset (`RSTSR1.WDTRF`) the duty starts at 100% so a reset loop still cools.
- HA integration is MQTT auto-discovery (publishes `fan` / `sensor` (temp, rpm) / `binary_sensor` + a `climate` HVAC entity carrying the mode presets), all with availability on `rackfan/<id>/avail` (retained `online` + LWT `offline`); HomeKit is reached by HA's HomeKit Bridge. `homeassistant/rack_fan.yaml` is a manual-YAML fallback for the same entities. Setpoint/mode/manual-% persist in EEPROM (`cfg`@0).
